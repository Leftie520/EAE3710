...
public class PrefabDB : MonoBehaviour
{
    ...
    public Dictionary<ShopItems, GameObject> shopItemsTable;
    public Dictionary<ShopItems, string> shopItemsDescs;
    ...
    [SerializeField]
    private GameObject[] shopItemsAsGOs;
    ...
    private void Awake()
    {
        ...
        // Initialize Shop Items Tables
        shopItemsTable = new Dictionary<ShopItems, GameObject>();
        shopItemsDescs = new Dictionary<ShopItems, string>();

        for (int i = 0; i < shopItemsAsGOs.Length; i++)
        {
            shopItemsTable.Add((ShopItems)i, shopItemsAsGOs[i]);
        }

        shopItemsDescs.Add(ShopItems.Moon, "Moon Bumper:\n\nA very large bumper which scores 100 points when hit.");
        shopItemsDescs.Add(ShopItems.Mercury, "Mercury Bumper:\n\nA bumper which launches the ball very fast and scores 500 points when hit.");
        shopItemsDescs.Add(ShopItems.Kepler, "Kepler Bumper:\n\nA bumper which scores 200 points on its first hit, 400 on its second, up to 2000 points, and explodes on its 5th hit.");
        shopItemsDescs.Add(ShopItems.Clone, "Clone Bumper:\n\nA bumper which spawns ghost balls when hit. Scores 100 points when hit.");
        shopItemsDescs.Add(ShopItems.Moblize, "Mobilize Bumper:\n\nA bumper which scores 150 points on first hit, 250 on second hit, and so on.");
        shopItemsDescs.Add(ShopItems.WhiteDwarf, "WhiteDwarf Bumper:\n\nA bumper which scores 250 points and adds 0.5 seconds of extra time when hit.");
        shopItemsDescs.Add(ShopItems.Venus, "Venus Bumper:\n\nA bumper that scores 600 points and randomly changes the bounce angle when hit.");
        shopItemsDescs.Add(ShopItems.Phobos, "Phobos Bumper:\n\nA bumper that scores 400 points and slightly shifts its position when hit.");
        shopItemsDescs.Add(ShopItems.CometTrail, "CometTrail Spinner:\n\nA spinner which scores 700 points and temporarily speeds up the ball when hit.");
        shopItemsDescs.Add(ShopItems.DarkMatter, "DarkMatter Spinner:\n\nA spinner Randomly grants 1000 points or deducts 500 points when hit.");
        shopItemsDescs.Add(ShopItems.Wormhole, "Wormhole Spinner:\n\nA spinner that teleports the ball to a random wormhole and scores 800 points when hit.");
    }
